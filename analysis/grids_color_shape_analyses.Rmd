...
library_results_subdirs = c(".", "meta_faster")
...
comparison_d = comparison_d %>%
    group_by(run_type, run, game_type, source_good_obj) %>%
...
```{r}
lmer(normalized_rewards ~ run_type + (1|run),
     comparison_d) %>%
  summary()
```
...
ggplot(data=comparison_d %>%
  ...
  labs(x="Model", y="Normalized returns (evaluation tasks)") +
